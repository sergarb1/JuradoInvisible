...
<template>
  <div class="rounded-xl bg-slate-900 p-4">
    <div class="mb-3 flex items-center justify-between">
      ...
      <button
        type="button"
        class="flex items-center gap-1 rounded-md px-2 py-1 text-xs text-slate-400 transition hover:bg-slate-800 hover:text-slate-200"
    ...
    <div v-if="helpOpen" class="mb-3 flex flex-col gap-2 rounded-lg bg-slate-950/60 p-3">
      <p v-for="row in rows" :key="row.key" class="text-xs text-slate-300">
        <span class="font-semibold text-slate-200">{{ row.label }}</span>: {{ row.desc }}
      </p>
    </div>
