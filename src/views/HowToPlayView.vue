...
<template>
  <main class="mx-auto flex min-h-svh max-w-2xl flex-col gap-8 p-6">
    <button
      type="button"
      class="flex w-fit items-center gap-2 text-sm text-slate-400 transition hover:text-slate-200"
      @click="router.back()"
    >
    ...
    <header>
      ...
      <p class="mt-2 text-slate-400">
        «El Jurado Invisible» es un <strong class="text-slate-200">simulador social</strong>: tomas
    ...
    <section class="flex flex-col gap-4">
      <h2 class="text-sm font-semibold uppercase tracking-wide text-slate-400">El bucle del día</h2>
      <div
        v-for="(paso, i) in PASOS"
        :key="paso.title"
        class="flex gap-4 rounded-xl bg-slate-900 p-4"
      >
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-slate-400">{{ paso.text }}</p>
        </div>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="flex flex-col gap-3">
        <div
          ...
          :key="k"
          class="flex items-start gap-3 rounded-xl bg-slate-900 p-4"
        >
          <component
          ...
          />
          <div>
            <p class="font-semibold text-slate-100">{{ VARIABLE_META[k].label }}</p>
            <p class="text-sm text-slate-400">{{ VARIABLE_META[k].desc }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section>
      ...
      <div class="flex flex-col gap-3">
        <div
          ...
          class="flex items-start gap-3 rounded-xl bg-slate-900 p-4"
        >
          <component :is="ROLE_META[role].icon" class="mt-0.5 h-5 w-5 shrink-0 text-slate-300" />
          <div>
            <p class="font-semibold text-slate-100">{{ ROLE_META[role].label }}</p>
            <p class="text-sm text-slate-400">{{ ROLE_META[role].desc }}</p>
          </div>
        </div>
      </div>
    </section>
